...
const BlogReactions = ({ _id, mood, reactions }: Pick<Post, '_id' | 'mood'> & { reactions?: number[] }) => {
    const mouseY = useMotionValue(Infinity)
    const onMouseMove = useCallback((e: React.MouseEvent) => {
        mouseY.set(e.clientY)
    },
        [mouseY])
    const [cachedReactions, setCachedReactions] = useState(reactions ?? [0, 0, 0, 0])

    const onClick = useCallback(async (index: number) => {
        // Optimistic update
